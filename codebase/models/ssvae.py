...
class SSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        # print(torch.all(x[0]==x[x.shape[0]//self.y_dim]))
        # print(x.shape, y.shape)
        kl_y = ut.kl_cat(y_prob, y_logprob, -np.log(self.y_dim)).mean()
        y_prob = y_prob.transpose(0,1).reshape(-1)
        m, v = self.enc(x, y)
        kl_z = (y_prob * ut.kl_normal(m, v, self.z_prior_m, self.z_prior_v)).mean() * self.y_dim
        z = ut.sample_gaussian(m,v)
        rec = -(y_prob * ut.log_bernoulli_with_logits(x, self.dec(z, y))).mean() * self.y_dim
        nelbo = kl_y + kl_z + rec
        ################################################################################
